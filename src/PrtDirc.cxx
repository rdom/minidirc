// simulation software for the Mini DIRC prototype
...
int main(int argc, char **argv) {
  ...
  TApplication theApp("MiniDirc", 0, 0);
  ...
  for (int i = 1; i < argc; i = i + 2) {
    ...
    else if (G4String(argv[i]) == "-ionz") ionz = atoi(argv[i + 1]);
    else {
  ...
  if (runtype < 2) {
    ...
    if (momentum < 1e-20) {
      ...
      else if (particle == "ion") momentum = 9000;
      else momentum = 6;
    }
    if (runtype == 1) {
  ...
  if (particle == "ion") pdgid = 1234;
  if (particle == "mix_ion") pdgid = 10006;
